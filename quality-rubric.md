...
| Uncertainty gaming (stress harness) | Critical | On `modern_*` benchmark evaluators: `[illegible]` / `[gap]` / `[damaged]` / `[crop]` aligned to >15% of GT words, or wildcards with zero substantive matches — treated as skipping work, not honest damage reporting ([`benchmark/stress_metrics.py`](benchmark/stress_metrics.py)). Honest single-token gaps do not count as omissions. |
| Expansion-mode mismatch (stress harness) | Critical | YAML with `preserveOriginalAbbreviations: true` scored against expanded GT (`medieval`, `legal`) — row fails expansion firewall; not scored ([`benchmark/stress_gate.py`](benchmark/stress_gate.py), protocol §2.4.1). |
